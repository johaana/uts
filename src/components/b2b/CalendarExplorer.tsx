...
export function CalendarExplorer() {
  ...
  const [lens, setLens] = useState('holidays');
      ...
      <div className="max-w-6xl mx-auto space-y-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
             <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#E94368] font-ui text-left">Explorer</p>
             <h2 className="text-4xl md:text-6xl font-bold font-display tracking-tight text-[#17151A] text-left">The Calendar Explorer</h2>
          </div>
          
          <div className="flex flex-wrap gap-4">
             <div className="flex p-1 bg-[#F7F4EE] border border-[#DED9D0] rounded-sm">
        ...
        <div className="flex items-center gap-6 overflow-x-auto pb-4 border-b border-[#DED9D0]">
          {['Holidays', 'Religious', 'Cultural', 'Public', 'Working Days', 'Long Weekends'].map(l => (
            <button 
              key={l}
              onClick={() => setLens(l.toLowerCase())}
              className={cn(
                "whitespace-nowrap text-[11px] font-bold uppercase tracking-[0.2em] transition-colors",
                lens === l.toLowerCase() ? "text-[#E94368]" : "text-[#6D6870] hover:text-[#17151A]"
              )}
            >
              {l}
            </button>
        ...
        <div className="bg-white border border-[#DED9D0] p-8 md:p-12 shadow-sm rounded-sm">
          ...
          {view === 'list' && (
            <div className="space-y-6">
              {[
                { date: '02 OCT', name: 'Gandhi Jayanti', type: 'Public Holiday' },
                { date: '12 OCT', name: 'Navratri Starts', type: 'Religious' },
                { date: '29 OCT', name: 'Diwali', type: 'Public Holiday' }
              ].map(event => (
                <div key={event.name} className="flex items-center gap-8 p-6 border-b border-[#F7F4EE] hover:bg-[#F7F4EE]/50 group transition-colors">
                  <span className="font-data text-sm font-bold w-16">{event.date}</span>
                  <div className="h-4 w-[1px] bg-[#DED9D0]" />
                  <span className="font-display text-xl font-bold flex-1">{event.name}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#6D6870]">{event.type}</span>
                  <button className="text-[10px] font-bold uppercase tracking-widest text-[#E94368] opacity-0 group-hover:opacity-100 transition-opacity">Record →</button>
                </div>
              ))}
            </div>
          )}
