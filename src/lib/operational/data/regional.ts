/**
 * @fileOverview Structured Regional Intelligence Registry.
 * CA example provided; others extracted from chunks.
 */
import { DateIntelligenceRecord } from '../types';

export const REGIONAL_INTELLIGENCE: any[] = [
  {
    country: "CA",
    date: "2026-02-16",
    region: "BC / AB / SK / MB / ON / NB / NS / PEI",
    text: "Provincial Family/Heritage/Islander holiday signals occur across several provinces; statutory treatment varies by province."
  },
  {
    country: "IN",
    date: "2026-09-25",
    region: "Maharashtra",
    text: "Anant Chaturdashi is listed in Maharashtra's 2026 holiday calendar; local operational treatment can vary by institution."
  }
];
